...
export default class AdmissionComponentsPage extends AlertPage {
  title: ElementFinder = element(by.id('admission-heading'));
...
export class AdmissionDeleteDialog {
  dialog: ElementFinder = element(by.id('removeEntity'));
  title: ElementFinder = element(by.id('hackatonappApp.admission.delete.question'));
  confirmButton: ElementFinder = element(by.id('jhi-confirm-delete-admission'));
}
